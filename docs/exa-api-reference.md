...
>
> **How this codebase uses it:** All agents in `src/lib/agents/` call `/search` (with `outputSchema` for synthesis) via the wrappers in `src/lib/exa.ts`. Streaming agents (memo) use `/search?stream=true` and parse OpenAI-compatible chat-completion chunks via `src/lib/sse.ts`. We do **not** use the legacy `/answer` endpoint.
